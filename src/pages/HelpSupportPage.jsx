...
const FAQS = [
  {
    ...
    questions: [
      {
        q: `What is ${APP_NAME}?`,
        a: `${APP_NAME} is an educational platform where students earn coins by completing quizzes and tasks. These coins can be used to purchase rewards from the shop.`,
      },
      {
...
const CONTACT_OPTIONS = [
  ...
  },
  {
    icon: FaPhone,
    label: "Phone Support",
    description: SUPPORT_PHONE ? "Call our support line" : "Phone support unavailable",
    action: SUPPORT_PHONE,
    detail: SUPPORT_PHONE_LABEL,
    color: "bg-green-500",
    type: "phone",
  },
  {
...
export default function HelpSupportPage() {
  ...
  const handleContactSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        `❌ ${error.message || `Failed to send email. Please contact ${SUPPORT_EMAIL}.`}`,
        "error"
      );
      setSubmitted(false);
    }
  ...
  const handleContactOptionClick = (option) => {
    if (option.type === "email") {
      window.location.href = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(`${APP_NAME} Support Request`)}`;
      return;
    }
              ...
              <div className="gap-2 grid grid-cols-3">
                {CONTACT_OPTIONS.map((option) => (
                  ...
                  >
                    ...
                    <span className="text-[9px] text-slate-400 dark:text-slate-500 text-center">{option.detail || option.description}</span>
                  </button>
                ))}
              </div>
